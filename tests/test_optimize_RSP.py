...
# =============================================================================
# Constraint Verification Tests (Stage 4 verification - verification-plan.md)
# =============================================================================

class TestConstraintSatisfactionAfterSolve:
    """
    Tests to verify that constraints remain satisfied AFTER the optimization solve.

    This addresses the bug mentioned in verification-plan.md line 153:
    "Previous bug found: Normalization after solve broke constraints."

    The bug was that centering u,v after the solve (u = u - mean(u)) broke
    constraint satisfaction because the constraints involve sign bits s[c]
    that can be +1 or -1, so shifting v doesn't cancel properly.
    """

    def test_solve_constraints_only_preserves_feasibility(self):
        """
        Test that solve_constraints_only returns a solution that satisfies constraints.

        This test was added as part of Stage 4 verification (verification-plan.md).
        The bug was that normalization (centering) after the solve broke constraints.
        """
        ...
        # Stage 2: Cross field
        W, xi = compute_smooth_cross_field(mesh, smoothing_iters=100, verbose=False)

        # Stage 3: Cut graph
        alpha = compute_corner_angles(mesh)
        Gamma, zeta, s, phi, omega0 = compute_cut_jump_data(mesh, alpha, xi)

        # Stage 4: Solve constraints
        u, v, theta = solve_constraints_only(mesh, alpha, phi, omega0, s, max_iters=500, verbose=False)

        # VERIFICATION: Check that constraints are satisfied AFTER the solve
        F = build_constraint_system(mesh, alpha, u, v, s, phi, theta, omega0)
        constraint_norm = np.linalg.norm(F)
        max_constraint = np.max(np.abs(F)) if len(F) > 0 else 0.0

        # The solver should achieve constraint satisfaction to within tolerance
        # If normalization broke constraints, this would fail
        assert constraint_norm < 1e-5, \
            f"Constraints violated after solve! L2 norm = {constraint_norm:.6e} (expected < 1e-5)"
        assert max_constraint < 1e-4, \
            f"Constraints violated after solve! Max violation = {max_constraint:.6e} (expected < 1e-4)"

    def test_no_normalization_after_solve(self):
        """
        Verify that optimization.py solve_constraints_only does NOT normalize u,v after solve.

        This is a static analysis test - checking the source code itself.
        The bug was that the code had:
            u = u - np.mean(u)
            v = v - np.mean(v)
        after the solve loop, which broke constraint satisfaction.
        """
        import inspect
        from optimization import solve_constraints_only

        source = inspect.getsource(solve_constraints_only)

        # Check that the dangerous patterns are NOT present
        assert "u = u - np.mean(u)" not in source, \
            "BUG: Normalization 'u = u - np.mean(u)' found after solve - this breaks constraints!"
        assert "v = v - np.mean(v)" not in source, \
            "BUG: Normalization 'v = v - np.mean(v)' found after solve - this breaks constraints!"
        assert "u -= np.mean(u)" not in source, \
            "BUG: Normalization 'u -= np.mean(u)' found after solve - this breaks constraints!"
        assert "v -= np.mean(v)" not in source, \
            "BUG: Normalization 'v -= np.mean(v)' found after solve - this breaks constraints!"

        # Check that the NOTE about not centering is present (documentation)
        assert "Do NOT center u,v" in source or "NOTE" in source, \
            "Missing documentation note about not centering u,v after solve"
