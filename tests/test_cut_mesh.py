...
# =============================================================================
# Test: Integration with Real Mesh Files
# =============================================================================

class TestRealMeshes:
    """Integration tests with real mesh files."""

    @pytest.fixture
    def sphere_mesh_path(self):
        """Path to sphere mesh."""
        path = Path("C:/Dev/Colonel/Data/Meshes/sphere320.obj")
        if not path.exists():
            pytest.skip(f"Sphere mesh not found: {path}")
        return str(path)

    def test_sphere_cut_to_disk(self, sphere_mesh_path):
        """Cutting sphere should produce disk topology."""
        ...
        mesh = load_obj(sphere_mesh_path)
        ...
        # Build mesh info from connectivity
        info = _build_meshinfo(mesh.positions, mesh.faces)

        # Get singularity vertex indices
        sing_verts = np.where(np.abs(cone_indices) > 0.1)[0]
        ...
        chi_cut = SrcCut.nf - SrcCut.ne + SrcCut.nv
        assert chi_cut == 1, f"Cut sphere should have chi=1 (disk), got {chi_cut}"

    def test_sphere_face_count_preserved(self, sphere_mesh_path):
        """Face count should be preserved after cutting."""
        from io_obj import load_obj
        from geometry import compute_corner_angles
        from cross_field import compute_smooth_cross_field, compute_cross_field_singularities
        ...
        assert SrcCut.nf == mesh.n_faces, \
            f"Face count should be preserved: {SrcCut.nf} vs {mesh.n_faces}"
