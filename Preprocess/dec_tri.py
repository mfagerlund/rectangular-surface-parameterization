...
def dec_tri(Src: MeshInfo) -> DEC:
    ...
    half_area = np.tile(Src.area[:, np.newaxis] / 3, (1, 3))  # shape (nf, 3)
    ...
    cot_reordered = Src.cot_corner_angle[:, [2, 0, 1]]  # shape (nf, 3)
    ...
    T2E_flat = (np.abs(Src.T2E) - 1).flatten('F')  # shape (3*nf,), 0-based edge indices
    ...
    col_idx = np.concatenate([Src.E2V[:, 0], Src.E2V[:, 1]])
    ...
    col_idx = (np.abs(Src.T2E) - 1).flatten()  # 0-based edge indices
    data = np.sign(Src.T2E).flatten()  # edge orientations
    ...
    star2p = diags(1.0 / Src.area, format='csr')
    ...
    star0d = diags(Src.area, format='csr')
    ...
    # T2E uses 1-based signed encoding, so abs(T2E)-1 gives 0-based edge indices
    T2E_abs_flat = (np.abs(Src.T2E) - 1).flatten()
    ...
    I = np.abs(Src.T2E[:, [0, 1, 2]]) - 1  # shape (nf, 3), 0-based edge indices
    ...
    T2E_sign = np.sign(Src.T2E[:, [0, 1, 2]])  # shape (nf, 3)
    T2E_abs = np.abs(Src.T2E[:, [0, 1, 2]]) - 1  # shape (nf, 3), 0-based edge indices
